...
Data was downloadeed from AttomData.com, a commercial data provider, using their RESTful API and
...
The the data was filtered to remove property types weren't residential condos or homes.
...
Here's a link [back to the Summary](#Summary) section of this report

After downloaind the raw data, we had sales information covering the following data range:
```{r summary_date_range, echo=FALSE, include=TRUE,eval=TRUE, cache=TRUE}
```

We found additional filter criteria to help use cleanse the data.
For example, below the raw data has been filter to show just the property types and subtypes of interest,
...
Here's a summary of the loaded data by year
<!-- consider changing to a bar chart -->
```{r summary_by_year, echo=FALSE, include=TRUE, cache=TRUE}
